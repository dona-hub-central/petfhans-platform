...
export default async function OwnerSettingsPage({
  ...
  async function saveProfile(formData: FormData) {
    ...
    if (updateErr) redirect('/owner/settings?error=save')
    revalidatePath('/owner/settings')
    revalidatePath('/owner/dashboard')
    redirect('/owner/settings?success=profile')
  }
  ...
  async function changePassword(formData: FormData) {
    ...
    if (!current || !next || !confirm) redirect('/owner/settings?error=fields')
    if (next !== confirm)              redirect('/owner/settings?error=match')
    if (next.length < 8)              redirect('/owner/settings?error=short')
    const sb = await createClient()
    const { data: { user: u } } = await sb.auth.getUser()
    if (!u) redirect('/auth/login')
    const { error: verifyErr } = await sb.auth.signInWithPassword({ email: u.email!, password: current })
    if (verifyErr) redirect('/owner/settings?error=wrong')
    const { error: updErr } = await sb.auth.updateUser({ password: next })
    if (updErr) redirect('/owner/settings?error=update')
    redirect('/owner/settings?success=password')
  }
      ...
      <div className="prof">
        <div className="prof-header">
          <div className="prof-header-inner">
            ...
            <h1 className="prof-title">Mi cuenta</h1>
          </div>
        ...
        <div className="prof-body">
          ...
          <Link href="/owner/support" className="card" style={{ display:'flex', alignItems:'center', gap:14, textDecoration:'none' }}>
            <div style={{
              ...
              background:'var(--pf-warning)', display:'flex', alignItems:'center', justifyContent:'center',
              color:'var(--pf-warning-fg)', fontSize:22, fontWeight:700,
            }}>?</div>
