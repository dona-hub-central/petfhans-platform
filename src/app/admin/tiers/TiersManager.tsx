    ...
    <div>
        ...
        <div className="bg-white rounded-2xl border p-6 mb-6" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="col-span-2">
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Nombre del tramo *</label>
              <input value={form.name ?? ''} onChange={e => set('name', e.target.value)}
                placeholder="Ej: Starter, Básico, Estándar…" className={inp} style={inpS} {...f} />
            </div>
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Desde (pacientes)</label>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Hasta (pacientes) <span style={{fontWeight:400}}>— vacío = ilimitado</span></label>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Precio mensual (€) *</label>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Precio anual (€) <span style={{fontWeight:400}}>— opcional</span></label>
            ...
            <div className="col-span-2">
              <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--muted)' }}>Descripción</label>
            ...
            <div className="col-span-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm" style={{ color: 'var(--text)' }}>Tarifa activa</span>
              </label>
            </div>
          </div>
      ...
      {tiers.length === 0 ? (
        <div className="bg-white rounded-2xl border p-12 text-center" style={{ borderColor: 'var(--border)' }}>
          ...
          <p className="text-sm" style={{ color: 'var(--muted)' }}>Sin tarifas definidas. Crea la primera.</p>
        </div>
      ) : (
            ...
            <thead>
              <tr style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
                {['Nombre', 'Tramo de pacientes', '€/mes', '€/año', 'Estado', 'Acciones'].map(h => (
                  <th key={h} className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--muted)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: 'var(--border)' }}>
              {tiers.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-5 py-4">
                    <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: 'var(--accent-s)', color: 'var(--accent)' }}>
                      {t.min_patients} – {t.max_patients ?? '∞'}
                    </span>
                  </td>
                  <td className="px-5 py-4 font-bold" style={{ color: 'var(--text)' }}>{t.price_monthly}€</td>
                  <td className="px-5 py-4" style={{ color: 'var(--muted)' }}>{t.price_yearly ? `${t.price_yearly}€` : '—'}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(t)} className="text-xs font-medium" style={{ color: 'var(--accent)' }}>Editar</button>
