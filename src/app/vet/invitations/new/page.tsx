    ...
    <div className="min-h-screen flex items-start justify-center p-8" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-lg">
        <div className="bg-white rounded-2xl border p-8" style={{ borderColor: 'var(--border)' }}>
          <a href="/vet/invitations" className="text-xs mb-6 inline-block" style={{ color: 'var(--muted)' }}>← Invitaciones</a>
          <h1 className="text-xl font-bold mb-1" style={{ color: 'var(--text)' }}>Nueva invitación</h1>
          <p className="text-sm mb-6" style={{ color: 'var(--muted)' }}>El invitado recibirá un link de acceso</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>Rol *</label>
                ...
                {roles.map(r => (
                  <button key={r.value} type="button" onClick={() => set('role', r.value)}
                    ...
                    style={{
                      ...
                      background: form.role === r.value ? 'var(--accent-s)' : '#fff',
                    }}>
                    <p className="text-sm font-medium" style={{ color: form.role === r.value ? 'var(--accent)' : 'var(--text)' }}>
                      {r.label}
                    </p>
