...
export default function NewPetPage() {
  ...
  return (
    <div className="min-h-screen flex items-start justify-center p-8" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-2xl">
        <div className="bg-white rounded-2xl border p-8" style={{ borderColor: 'var(--border)' }}>
          <a href="/vet/pets" className="text-xs mb-6 inline-block" style={{ color: 'var(--muted)' }}>← Mascotas</a>
          <h1 className="text-xl font-bold mb-1" style={{ color: 'var(--text)' }}>Nueva mascota</h1>
          <p className="text-sm mb-6" style={{ color: 'var(--muted)' }}>Registra un nuevo paciente</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Especie */}
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>Especie</label>
              <div className="flex gap-2 flex-wrap">
                {species.map(s => (
                  <button key={s.value} type="button"
                    ...
                    style={{
                      borderColor: form.species === s.value ? 'var(--accent)' : 'var(--border)',
                      background: form.species === s.value ? 'var(--accent-s)' : '#fff',
                      color: form.species === s.value ? 'var(--accent)' : 'var(--muted)',
                    }}>
                    {s.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text)' }}>Nombre *</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text)' }}>Sexo</label>
                <div className="flex gap-2">
                  {[{ v: 'male', l: '♂ Macho' }, { v: 'female', l: '♀ Hembra' }].map(g => (
                    <button key={g.v} type="button" onClick={() => set('gender', g.v)}
                      className="flex-1 py-2.5 rounded-xl text-sm font-medium border transition"
                      style={{
                        borderColor: form.gender === g.v ? 'var(--accent)' : 'var(--border)',
                        background: form.gender === g.v ? 'var(--accent-s)' : '#fff',
                        color: form.gender === g.v ? 'var(--accent)' : 'var(--muted)',
                      }}>
                      {g.l}
                    </button>
                  ))}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text)' }}>Fecha de nacimiento</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text)' }}>Peso (kg)</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text)' }}>Microchip</label>
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => set('neutered', !form.neutered)}
                ...
                style={{ background: form.neutered ? 'var(--accent)' : 'var(--border)' }}>
