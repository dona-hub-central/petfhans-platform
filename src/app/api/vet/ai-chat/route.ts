...
export async function POST(req: NextRequest) {
  try {
    ...
    // Cargar agente IA configurado
    const { data: agent } = await admin.from('ai_agent').select('*').eq('id', 'default').single()
    if (!agent?.openai_api_key) {
      return NextResponse.json({ reply: 'El agente IA no está configurado. Ve a Admin → 🤖 Agente IA y añade la API key de OpenAI.' })
    }
    if (!agent.is_active) {
      return NextResponse.json({ reply: 'El agente IA está inactivo. Actívalo desde Admin → 🤖 Agente IA.' })
    }
...
${records?.map(r => `[${r.visit_date}] ${r.reason} → Dx: ${r.diagnosis ?? 'N/D'} · Tto: ${r.treatment ?? 'N/D'}${Array.isArray(r.medications) && r.medications.length > 0 ? ` · Meds: ${r.medications.map((m:any) => m.name).join(', ')}` : ''}`).join('\n') ?? 'Sin consultas'}`
      }
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ reply: `Error de IA: ${err.error?.message ?? 'Revisa la API key'}` })
    }
    ...
    return NextResponse.json({ reply: 'Error al conectar con la IA: ' + err.message })
  }
}
